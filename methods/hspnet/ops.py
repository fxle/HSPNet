...
class ConvGNReLU(nn.Sequential):
    def __init__(
        ...
        """
        Execution process Conv2d => GroupNormalization [=> Activation]

        Args:
        in_planes: number of module input channels
        out_planes: number of module output channels
        kernel_size: convolution kernel size for internal convolution operation
        stride: convolution step length
        padding: convolution padding
        dilation: dilation rate of convolution
        groups: number of convolution groups, which must meet the requirements of pytorch itself
        gn_groups: number of GroupNormalization groups, default is 4
        bias: whether to enable convolution bias, default is False
        act_name: activation function used, default is relu, when set to None, no activation function is used
        inplace: set the inplace parameter of the activation function
        """
        super().__init__()
        self.in_planes = in_planes
        self.out_planes = out_planes
...
class LayerNorm2d(nn.Module):
    """
    From detectron2 of facebookresearch
    """
