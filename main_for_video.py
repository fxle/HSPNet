...
def train(model, cfg):
    ...
    for epoch in range(counter.num_epochs):
        ...
        if (epoch + 1) % 1 == 0:
            test_wrapper = Evaluator(device=cfg.device, metric_names=cfg.metric_names, clip_range=cfg.test.clip_range)
            te_name = cfg.test.data.names[0]  
            te_info = cfg.dataset_infos[te_name]
            te_dataset = VideoTestDataset(dataset_info=te_info, shape=cfg.test.data.shape, num_frames=cfg.num_frames)
            te_loader = data.DataLoader(
                dataset=te_dataset, batch_size=cfg.test.batch_size, num_workers=cfg.test.num_workers, pin_memory=True
            )
            seg_results, _ = test_wrapper.eval(model=model, data_loader=te_loader)
            seg_results_str = ", ".join([f"{k}: {v:.03f}" for k, v in seg_results.items()])
            LOGGER.info(f"({te_name}): {py_utils.mapping_to_str(te_info)}\n{seg_results_str}")
            metrics = ['sm', 'wfm', 'avgem', 'avgiou', 'avgdice']
            metric_values = [seg_results.get(metric, 0.0) for metric in metrics]
            current_metrics = np.mean(metric_values)

            if current_metrics > final_metrics:
    ...
    LOGGER.info(f"Total Training Time: {datetime.timedelta(seconds=int(total_train_time))} ({total_other_time} on others)")



def parse_cfg():
    parser = argparse.ArgumentParser("Training and evaluation script")
    parser.add_argument("--config", default='configs/vcod_finetune.py', type=str)
    parser.add_argument("--data-cfg", type=str, default="./dataset.yaml")
    parser.add_argument("--model-name", type=str, default='videoPvtV2B5_HSPNet')
    parser.add_argument("--output-dir", type=str, default="outputs")
    parser.add_argument("--load-from", type=str)
    # parser.add_argument("--load-from", type=str, default='outputs/exp_0/pth/state_final.pth')
    ...
    parser.add_argument("--info", type=str, default='finetune')
...
def main():
    ...
    if cfg.load_from:
        ...
        # io.load_weight(model=model, load_path=cfg.load_from, strict=False, skip_unmatched_shape = True)
    ...
    if cfg.evaluate or cfg.has_test:
        io.save_weight(model=model, save_path=cfg.path.best_state_net)
