...
def group_params(model: nn.Module, group_mode: str, initial_lr: float, optim_cfg: dict):
    if group_mode == "yolov5":
        ...
        params = [
            # Do not perform weight decay on the bias parameter. The main function of weight decay is to reduce the overfitting of the model by constraining the parameters of the network
            # layer (including weight and bias) (L2 regularization will make the parameters of the network layer smoother).。
            {
                "params": [
