...
def construct_exp_name(model_name: str, cfg: dict):
    ...
    def _format_item(_i):
        if isinstance(_i, bool):
            ...
            _i = "" if _i else "false"  # Just check if it is not empty
...
def pre_mkdir(path_config):
    # Create the record file in advance to avoid triggering the file creation event when it is automatically created
    check_mkdir(path_config["pth_log"])
    make_log(path_config["log"], f"=== log {datetime.now()} ===")

    # Create folders in advance to store prediction results and models
...
def iterate_nested_sequence(nested_sequence):
    """
    Currently supports multiple nesting of list/tuple/int/float/range(). Be careful not to nest too deeply and be careful not to exceed the maximum recursive depth of Python by default
    ...
    :param nested_sequence: multiple nested sequences
    :return: generator
    """
