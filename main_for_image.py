...
def train(model, cfg):
    ...
    for epoch in range(counter.num_epochs):
        ...
        if (epoch + 1) % 1 == 0:
            ...
            seg_results = test_wrapper.eval(model=model, data_loader=te_loader)
            seg_results_str = ", ".join([f"{k}: {v:.03f}" for k, v in seg_results.items()])
            LOGGER.info(f"({te_name}): {py_utils.mapping_to_str(te_info)}\n{seg_results_str}")
            metrics = ['sm', 'wfm', 'avgem']
...
def parse_cfg():
    ...
    parser.add_argument("--config", default='configs/icod_pretrain.py', type=str)
